...
class TextField: UIView {
    ...
    var text: String = "" {
        willSet {
            if newValue.count != 0, text == "" { textEntered() }
            else if text != "", newValue.count == 0 { if needPlaceholder { textRemoved() } }
        }
    }
    ...
    func makeFirstResponder() { let _ = textField.becomeFirstResponder() }
    ...
    private func addPasswordEye() {
        ...
        passwordEyeButton.frame = CGRect(x: CGFloat(textField.frame.size.width - 25), y: CGFloat(5), width: CGFloat(25), height: CGFloat(25))
...
extension TextField: UITextFieldDelegate {
    ...
    func textField(
        ...
        if type == .password && string == " " { return false }
        else { return true }
    }
}
